...
### Segment composition
To visualize composition of each cluster - we create a table of each cluster. First we inspect by country.
...
Estimating partworth utilities by cluster:
```{r, fig.width=16, fig.height=14}
...
To visualize relative importance by cluster, we can sort descending:
```{r, fig.width=16, fig.height=14}
...
Finally, computing importance by each cluster:
```{r}
...
Computing market share only for Japanese nationality:
```{r}
